...
export default function HomePage() {
  ...
  async function researchCompany() {
    ...
    setError('');
    try {
      const response = await fetch('/api/research', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ companyUrl: form.companyUrl, roleTitle: kit.role.title, requirements: kit.role.requirements, includeQuestions: false }) });
      const data = await response.json();
          ...
          {!kit ? (
            ...
            <div className="output-stack grid gap-6">
              ...
              <div className="panel border border-slate-200 bg-white p-5 shadow-sm">
                ...
                {kit.company_brief.interview_process && <div className="mt-4 border-l-4 border-amber-400 bg-amber-50 px-3 py-3"><p className="m-0 text-xs font-bold uppercase tracking-wide text-amber-900">Public interview signals</p><textarea aria-label="Public interview signals" value={kit.company_brief.interview_process.summary} onChange={(event) => updateBrief({ interview_process: { ...kit.company_brief.interview_process, summary: event.target.value } })} className="mt-2 min-h-16 w-full resize-y border border-amber-200 bg-white p-3 text-sm leading-6 text-slate-700" />{kit.company_brief.interview_process.sources?.length > 0 && <ul className="mb-0 mt-3 grid list-none gap-2 p-0" aria-label="Public interview discussion sources">{kit.company_brief.interview_process.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noreferrer" className="text-sm font-semibold text-mint underline">{source.title || source.url}</a><p className="m-0 mt-1 text-xs text-amber-950">{source.role_relevance === 'exact-role' ? 'Role-specific discussion' : source.role_relevance === 'related-role' ? 'Related-role discussion' : 'Company-wide discussion, not role-specific'}</p></li>)}</ul>}<p className="mb-0 mt-2 text-xs leading-5 text-amber-950">Community discussion is supplementary and is not treated as verified company policy.</p></div>}
              </div>
